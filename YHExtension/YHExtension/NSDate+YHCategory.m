...
#import "NSDate+YHCategory.h"
#import "NSDateFormatter+YHCategory.h"
...
@implementation NSDate (YHCategory)
...
/*距离当前的时间间隔描述*/
- (NSString *)timeIntervalDescriptionType1
{
    NSTimeInterval timeInterval = -[self timeIntervalSinceNow];
    if (timeInterval < 60) {
        return @"1分钟";
    } else if (timeInterval < 3600) {
        return [NSString stringWithFormat:@"%.0f分钟", timeInterval / 60];
    } else if (timeInterval < 86400) {
        return [NSString stringWithFormat:@"%.0f小时%lld分钟", timeInterval / 3600,((long long)timeInterval % 3600)/60];
    } else if (timeInterval < 2592000) {//30天内
        return [NSString stringWithFormat:NSLocalizedString(@"NSDateCategory.text4", @""), timeInterval / 86400];
    } else if (timeInterval < 31536000) {//30天至1年内
        NSDateFormatter *dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"NSDateCategory.text5", @"")];
...
- (NSString *)formattedDateDescription
{
    ...
    if (timeInterval < 60) {
        ...
        return [NSString stringWithFormat:NSLocalizedString(@"NSDateCategory.text2", @""), timeInterval / 60.0];
...
- (NSString *)dateDescriptionWithyyyyMMddHHmmss
{
    return [self dateDescriptionWithFormat:@"yyyy-MM-dd HH:mm:ss"];
}

- (NSString *)dateDescriptionWithyyyyMMdd
{
    return [self dateDescriptionWithFormat:@"yyyy-MM-dd"];
}

- (NSString *)dateDescriptionWithyyyyMMdd1
{
    return [self dateDescriptionWithFormat:@"yyyy.MM.dd"];
}

- (NSString *)dateDescriptionWithFormat:(NSString *)format
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
